...
Введіть [cyan]help[/cyan] для списку команд або почніть ставити запитання!
        ...
        self.console.print("[bold geen] Cli runned. Waiting  for inpu...[/bold green]")

        # Основной цикл
        while True:
            try:
                # Ввод пользователя
                self.console.print("[bold yellow] Waiting  for inpu...[/bold yellow]")
                user_input = Prompt.ask("\[bold blue]🔍 Ваш запит[/bold blue]", default="", show_default=False)
                ...
                self.console.print("\[yellow]Завершення роботи...[/yellow]")
                break
            except EOFError:
                self.console.print("\[yellow]Завершення роботи...[/yellow]")
                break
            except Exception as e:
                self.console.print(f"\[red]Виникла помилка:[/red] {e}")
